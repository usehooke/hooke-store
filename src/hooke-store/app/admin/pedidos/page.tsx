...
export default function AdminOrdersPage() {
    ...
    const StatusBadge = ({ status }: { status: OrderStatus }) => {
        ...
        return (
            <span className={`px-2 py-1 text-[10px] uppercase font-bold tracking-widest border rounded-none ${colors[status]}`}>
    ...
    if (loading || !user) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-white font-sans">
                <p className="text-hooke-900 font-bold uppercase tracking-widest text-xs">Carregando painel de pedidos...</p>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-white p-8 font-sans pb-24">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 border-b border-hooke-900 pb-6 gap-4">
                    <div>
                        <h1 className="text-3xl font-black uppercase tracking-tighter text-hooke-900">Gestão de Pedidos</h1>
                        <p className="text-xs uppercase tracking-widest text-gray-400 mt-2">Visão geral logística</p>
                    </div>
                ...
                <div className="flex gap-4 mb-8">
                    <button
                        ...
                        className={`text-xs font-bold uppercase tracking-widest px-6 py-3 transition-colors rounded-none border ${viewMode === 'all' ? 'bg-hooke-900 text-white border-hooke-900' : 'bg-transparent text-gray-500 border-gray-200 hover:border-gray-400'}`}
                    >
                        Todos os Pedidos
                    </button>
                    <button
                        onClick={() => setViewMode('opportunities')}
                        className={`text-xs font-bold uppercase tracking-widest px-6 py-3 transition-colors rounded-none border flex items-center gap-2 ${viewMode === 'opportunities' ? 'bg-green-600 text-white border-green-600' : 'bg-transparent text-gray-500 border-gray-200 hover:border-gray-400'}`}
                    >
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                        <thead>
                            <tr className="border-b border-hooke-900 bg-gray-50">
                                <th className="p-4 text-xs font-bold uppercase tracking-widest text-hooke-900">ID / Data</th>
                                <th className="p-4 text-xs font-bold uppercase tracking-widest text-hooke-900">Cliente</th>
                                <th className="p-4 text-xs font-bold uppercase tracking-widest text-hooke-900 w-[20%]">Itens</th>
                                <th className="p-4 text-xs font-bold uppercase tracking-widest text-hooke-900">Total / Pagto</th>
                                <th className="p-4 text-xs font-bold uppercase tracking-widest text-hooke-900 min-w-[150px]">Status</th>
                                <th className="p-4 text-xs font-bold uppercase tracking-widest text-hooke-900 min-w-[200px]">Rastreio</th>
                                    ...
                                    <tr key={order.id} className={`border-b border-gray-200 last:border-0 hover:bg-gray-50 transition-colors`}>
                                        <td className="p-4">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <span className="text-[10px] text-gray-500 uppercase">{dataFormatada}</span>
                                            </div>
                                        </td>
                                        <td className="p-4">
                                            <div className="flex flex-col gap-1">
                                        ...
                                        <td className="p-4">
                                            <div className="flex flex-col gap-1 items-start">
                                                ...
                                                {order.paymentMethod && <span className="text-[10px] uppercase bg-gray-200 px-2 py-0.5 mt-1">{order.paymentMethod}</span>}
                                                {order.shippingMethod && <span className="text-[10px] text-hooke-500 mt-1 uppercase font-bold tracking-widest">{order.shippingMethod}</span>}
                                            </div>
                                        </td>
                                        <td className="p-4">
                                            <select
                                                ...
                                                className="w-full bg-white border border-gray-300 rounded-none px-2 py-2 text-xs font-bold uppercase tracking-widest focus:ring-1 focus:ring-hooke-900 focus:border-hooke-900 outline-none transition-all text-hooke-900"
                                            >
                                        ...
                                        <td className="p-4 text-right align-middle">
                                            <div className="flex flex-col gap-2 w-full">
                                                <button
                                                    ...
                                                    className="bg-hooke-900 hover:bg-black text-white px-4 py-3 rounded-none text-[10px] font-bold uppercase tracking-widest transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 w-full"
                                                >
                                                ...
                                                {order.trackingCode && order.customer.phone && viewMode === 'all' && (
                                                    <button
                                                        ...
                                                        className="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-none text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 w-full"
                                                ...
                                                {viewMode === 'opportunities' && order.customer.phone && (
                                                    <button
                                                        ...
                                                        className="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-none text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 w-full animate-pulse"
