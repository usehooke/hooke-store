    ...
    <div className="min-h-screen bg-white text-black font-sans p-8 print:p-0">
      ...
      <header className="flex items-center justify-between mb-8 print:hidden">
        <div className="flex items-center gap-4">
          <Link href="/admin/pdv" className="p-3 shadow-neumorph rounded-full hover:bg-gray-100">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-2xl font-black uppercase tracking-tighter">Guia de SKUs Hooke 2026</h1>
        </div>
        <button 
          onClick={() => window.print()}
          className="flex items-center gap-2 bg-black text-white px-6 py-3 font-bold uppercase tracking-widest text-xs hover:bg-gray-800 transition-all"
        >
      ...
      <div className="max-w-4xl mx-auto border-2 border-black p-10 print:border-0 print:p-0">
        <div className="flex justify-between items-start border-b-4 border-black pb-6 mb-10">
          <div>
            <h2 className="text-4xl font-black uppercase tracking-tighter leading-none mb-2">Hooke</h2>
            <p className="text-sm font-bold tracking-widest uppercase opacity-60">Dicionário Oficial de SKUs 2026</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] font-black uppercase">Versão 1.0</p>
            <p className="text-[10px] font-black uppercase">Padrão: [MODELO]-[ESTAMPA]-[COR]-[TAMANHO]</p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 font-black uppercase text-[10px]">Sigla</th>
                  <th className="text-left py-2 font-black uppercase text-[10px]">Nome</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(MODEL_DICTIONARY).map(([sigla, info]) => (
                  <tr key={sigla} className="border-b border-gray-100 italic">
                    ...
                    <td className="py-2 uppercase font-medium">{info.label}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <footer className="mt-16 pt-6 border-t border-black flex justify-between items-center italic opacity-60">
          <p className="text-[10px] font-bold uppercase tracking-widest">Hooke Clothing Co. 2026</p>
          <div className="flex items-center gap-2">
            <FileText className="h-4 w-4" />
            <p className="text-[10px] font-bold uppercase tracking-widest">Guia de Codificação SKU</p>
          </div>
        </footer>
      </div>
