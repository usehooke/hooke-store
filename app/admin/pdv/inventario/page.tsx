      ...
      <div className="max-w-4xl mx-auto">
        <header className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-black tracking-tighter uppercase">Inventário Rápido</h1>
            </div>
            <button className="flex items-center gap-2 text-[10px] font-black uppercase text-hooke-500 shadow-neumorph px-4 py-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="md:col-span-1 bg-hooke-900 text-white p-6 shadow-neumorph flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-center">Aguardando Bip / Câmera Ativa</p>
                <input 
                    type="text" 
                    value={manualSku}
                    ...
                    className="w-full bg-white/10 p-3 text-xs border border-white/20 outline-none uppercase"
                />
            </div>

            <div className="md:col-span-2 bg-hooke-50 p-6 shadow-neumorph overflow-hidden">
                <h3 className="text-xs font-black uppercase tracking-widest mb-4">Resumo da Auditoria</h3>
                <div className="grid grid-cols-3 gap-4">
                    <div className="text-center font-black">
                        <p className="text-[10px] text-hooke-500 uppercase">Auditados</p>
                        <p className="text-2xl">{auditList.length}</p>
                    </div>
                    <div className="text-center font-black">
                        <p className="text-[10px] text-hooke-500 uppercase">Divergentes</p>
                        <p className="text-2xl text-red-500">{auditList.filter(i => i.actual !== i.expected).length}</p>
                    </div>
                    <div className="text-center font-black">
                        <p className="text-[10px] text-hooke-500 uppercase">Peças Totais</p>
        ...
        <div className="bg-hooke-50 shadow-neumorph overflow-hidden">
            <table className="w-full text-left">
                <thead className="bg-hooke-900/5 text-[10px] font-black uppercase tracking-widest">
                    <tr>
                ...
                <tbody className="text-xs font-bold divide-y divide-hooke-200">
                    {auditList.length === 0 ? (
                        <tr>
                            <td colSpan={4} className="p-12 text-center text-hooke-300 uppercase">Inicie o bipagem das peças...</td>
                        </tr>
                    ) : (
                        auditList.map((item) => (
                            <tr key={item.sku} className="hover:bg-hooke-100/30 transition-colors">
                                <td className="p-4">
                                    <p className="font-black truncate uppercase">{item.name}</p>
                                ...
                                <td className="p-4 text-center">
                                    {item.actual === item.expected ? (
                                        ...
                                        <div className="flex items-center justify-center text-red-500 gap-1">
                                            ...
                                            <span className="text-[10px] font-black uppercase">FALTA {item.expected - item.actual}</span>
                                        </div>
                                    )}
                                </td>
                            </tr>
        ...
        <div className="mt-8 flex gap-4">
            <button 
                ...
                className="flex-1 bg-white p-6 shadow-neumorph font-black uppercase tracking-widest text-xs active:shadow-neumorph-inset"
            >
                Limpar Lista
            </button>
            <button 
                className="flex-1 bg-hooke-900 text-white p-6 shadow-neumorph font-black uppercase tracking-widest text-xs active:scale-95 transition-all disabled:opacity-50"
